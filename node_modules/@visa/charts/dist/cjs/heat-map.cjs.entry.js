...
const quantize = require('./quantize-b181e2a6.js');
...
HeatMapDefaultValues.subTitle = propDefaultValues.subTitle.text;
...
HeatMapDefaultValues.localization = propDefaultValues.localization;
...
const HeatMapStyle0 = heatMapCss;
...
const HeatMap = class {
    ...
    componentWillLoad() {
        const chartID = this.uniqueID || 'heat-map-' + index$1.v4();
    ...
    prepareScales() {
        const minMapValue = (this.minValueOverride || this.minValueOverride === 0) &&
            this.minValueOverride < min.min(this.preppedData, d => d.valueAccessor)
            ? this.minValueOverride
            : min.min(this.preppedData, d => d.valueAccessor);
        const maxMapValue = (this.maxValueOverride || this.maxValueOverride === 0) &&
            this.maxValueOverride > max.max(this.preppedData, d => d.valueAccessor)
            ? this.maxValueOverride
            : max.max(this.preppedData, d => d.valueAccessor);
        this.x = band.band()
        ...
        this.heat = quantize.quantize().domain([minMapValue, maxMapValue]);
        const strokeColors = [];
        ...
        this.fillColors = quantize.quantize()
            .domain([minMapValue, maxMapValue])
            .range(backgroundColors);
        this.strokeColors = quantize.quantize()
            .domain([minMapValue, maxMapValue])
            .range(strokeColors);
        this.rawHeat = quantize.quantize()
            .domain([minMapValue, maxMapValue])
    ...
    renderRootElements() {
        this.svg = visaChartsUtils_umd.select(this.heatMapEl)
        ...
        this.legendG = visaChartsUtils_umd.select(this.heatMapEl)
            .select('.heat-map-legend')
            .append('svg');
        this.subTitleG = visaChartsUtils_umd.select(this.heatMapEl).select('.heat-sub-title');
        this.tooltipG = visaChartsUtils_umd.select(this.heatMapEl).select('.heat-map-tooltip');
    }
    setGlobalSelections() {
    ...
    setTestingAttributes() {
        if (this.unitTest) {
            visaChartsUtils_umd.select(this.heatMapEl)
                .select('.visa-viz-d3-heat-map-container')
    ...
    processLabelOpacity(selection, isATransition) {
        ...
        if (isATransition) {
            ...
            selection.each((d, i, n) => {
                if (!visaChartsUtils_umd.select(n[i]).classed('entering')) {
                    visaChartsUtils_umd.select(n[i]).attr('opacity', () => parseText(d));
                }
            });
        }
    }
    ...
    render() {
        ...
        // be moved into componentWillUpdate (if the Stencil bug is fixed)
        return (index.h("div", { key: '240f7f676fe22edbefe4d6cea36692509f86fa1f', class: `o-layout` }, index.h("div", { key: '879439bf0cdf6b0159cef9a40c2dbd8fe2f3d37d', class: "o-layout--chart" }, index.h(this.topLevel, { key: 'e4e8a187ff9525f42282c72f3778f20c99a1210f', class: "heat-main-title vcl-main-title" }, this.mainTitle), index.h(this.bottomLevel, { key: '31d5f0969d472b9924c24880e2a20a34c59121cc', class: "visa-ui-text--instructions heat-sub-title vcl-sub-title" }), index.h("div", { key: 'f9ef146256cfac4b11aedc9bc7fd8c0328c47de5', class: "heat-map-legend vcl-legend", style: { display: this.legend.visible ? 'block' : 'none' } }), index.h("keyboard-instructions", { key: '228a18ec888929e62c8442f45e04228323094d77', uniqueID: this.chartID, geomType: 'cell', groupName: 'row', language: this.getLanguageString(), chartTag: 'heat-map', width: this.width - (this.margin ? this.margin.right || 0 : 0), isInteractive: this.accessibility.elementsAreInterface, hasCousinNavigation: true, disabled: this.suppressEvents &&
                this.accessibility.elementsAreInterface === false &&
                this.accessibility.keyboardNavConfig &&
                this.accessibility.keyboardNavConfig.disabled }), index.h("div", { key: 'cca19b1a69d22fafca24c6370ec40366b842cad3', class: "visa-viz-d3-heat-map-container" }), index.h("div", { key: '0154056687d6429bcab1ed16d174a374b2a789d5', class: "heat-map-tooltip vcl-tooltip", style: { display: this.showTooltip ? 'block' : 'none' } }), index.h("data-table", { key: '0946e20c50c3f7f0395ceb159720b8302a9561c9', uniqueID: this.chartID, isCompact: true, language: this.getLanguageString(), tableColumns: this.tableColumns, data: this.tableData, dataKeyNames: this.dataKeyNames, padding: this.padding, margin: this.margin, hideDataTable: this.accessibility.hideDataTableButton, unitTest: this.unitTest }))));
    }
    init() {
        // reading properties
    ...
    get heatMapEl() { return index.getElement(this); }
...
HeatMap.style = HeatMapStyle0;

exports.heat_map = HeatMap;
